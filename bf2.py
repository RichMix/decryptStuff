...
# Verbose brute-force function with status updates
def brute_force_xor_key():
    ...
    for key_tuple in product(range(256), repeat=8):
        ...
        # Increment attempt counter and display status every 100,000 attempts
        attempt_count += 1
        if attempt_count % 100000 == 0:
            print(f"Attempt {attempt_count}: Trying XOR key {xor_key.hex()}...")
...
# Run the brute-force decryption with verbose output
brute_force_xor_key()
